...
모든 XML 구문을 허용하지만 DTD나 스키마를 사용하진 않는다.

#### 1-4-2. Vaild XML Doc

Well-formed에 추가로 DTD를 갖고, 그것에 따라 제대로 검증된 문서이다.

XML에서 사용하는 DTD 종류는 DTD, XSD 두 가지가 있다.

##### 1-4-2-1. DTD : Document Type Definition

XML에서 DTD를 작성하는 문법은 `<!DOCTYPE 루트요소 DTD식별자 [ 선언1 선언2 ...] >`이렇게 정의된다.

DTD는 구식 방법이지만 특유의 장점을 바탕으로 아직도 널리 이용된다.
...
XML 스키마 정의시에는 XSD 뿐만아니라 DTD도 사용할 수 있다.
...
> XSD에 대한 타입, 속성, 지시자 등 기본적인 개념들은 [이쪽]으로
